...
is to query data asynchonously, so the client isn't blocked while it waits for a
response from the server. Async queries can be performed using the
...
<Alert type="note">
  ...
  global scripts should be used. Don't let overly-dogmatic statements scare you
  away from the right solution - just be ready to explain and defend your
  reasoning.
</Alert>
