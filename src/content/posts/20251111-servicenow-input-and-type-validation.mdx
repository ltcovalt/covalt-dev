...
[Checker](/docs/servicenow/server/check) class to store the type validation, assertion,
...
server-side namespace named [Type](/docs/servicenow/server/type). The `Type` namespace
...
<Alert type="note">
	I originally exposed a factory in the [Type](/docs/servicenow/server/type) utility
	...
	[Check](/docs/servicenow/server/check) script include, but I think the original
...
In the end, we have a [Type](/docs/servicenow/server/type) namespace that includes
...
Next, we have the [Check](/docs/servicenow/server/check) namespace that defines the
