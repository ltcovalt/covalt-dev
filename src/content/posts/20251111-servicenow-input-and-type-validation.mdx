---
title: Input Validation and Type Checking in ServiceNow
slug: servicenow-input-and-type-validation
description:
    Build a fluent, chainable API for performing input validation and type
    checking in ServiceNow, using a standard JavaScript class and namespace
    facade.
author: 'Luke Covalt'
date: 2025-11-11
updated: 2025-11-11
...
the introduction of modern JavaScript to ServiceNow server-side scripts, I'm
...
As an example, I'd like to be able to do something like this:
...
// rather than calling standard functions
Check(value).not().isType('string');
```

You can also use the getters to define simple passthroughs, like 'is' (shown
...
<Alert type="note">
	...
	[Check](/code/servicenow/check) script include, but I think the original
	pattern was interesting enought to leave as a reference.
</Alert>
...
In the end, we have a [Type](/code/servicenow/type) namespace that includes
...
Next, we have the [Check](/code/servicenow/check) namespace that defines the
...
Neither util is anywhere near complete. There are many checks I want to add to
the `Checker` class and I want to make the logic for identifying the JavaObject
type much more robust in the `Type` utility. For now, they are functional enough
to start using and I'll add additional functionality as I need it.
