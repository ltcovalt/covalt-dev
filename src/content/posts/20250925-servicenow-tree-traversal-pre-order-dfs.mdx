---
title: "ServiceNow Tree Traversal: Pre-Order Depth-First Search"
slug: servicenow-tree-traversal-pre-order-depth-first-search
description:
  Implementing a pre order depth first search algorithm to traverse an
  n-ary/generic tree
author: "Luke Covalt"
date: 2025-09-25
updated: 2025-09-26
draft: false
tags:
  [blog, servicenow, tree, algorithm, recursion, pre-order, depth-first, dfs]
---

<Alert type="info">
	This post is part of a multi-part series on Tree Traversal  
	
	Previous: [Traverse to the Root](/posts/servicenow-tree-traversal-to-root-node) |  
	Next: [Post-Order Depth-First Search](/posts/servicenow-tree-traversal-post-order-depth-first)
...
traversing a branch from root to leaf before moving to the next branch. A
pre-order traversal is useful anytime you want to process parent nodes before
the children.
...
traversed, so performance will take a slight hit, particularly in extremely
large data sets. Since this is an indexed query for a single record, the
performance hit is negligible. Still, this is something you should keep in mind
if you plan to traverse a tree containing thousands of nodes. Ultimately, I
decided to avoid premature optimization and err on the side of caution,
