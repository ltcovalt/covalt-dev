...
branch. Each time we call our function, we push that node to the stack. When the
function completes, we remove the node from our stack. This allows the same node
to appear multiple times in the tree, but not multiple times in the same branch,
