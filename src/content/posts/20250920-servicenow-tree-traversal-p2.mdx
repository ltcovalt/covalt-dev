...
(() => {
   ...
   */
  Tree.walkToRoot = ({ node, parentField = "parent" }) => {
    ...
    let root = Tree.walkToRoot({ node: parentNode, parentField });
    return root;
  };
})();
```
...
(() => {
   ...
   * @param {string} [params.parentField=parent] - name of the parent reference field
   */
  Tree.walkToRoot = ({ node, parentField = "parent", stackPath = [] }) => {
    ...
    // get the parent node, then call the function again using the parent node
    let parentNode = node[parentField].getRefRecord();
    return Tree.walkToRoot({ node: parentNode, parentField, stackPath });
  };
})();
```
...
`Tree.walkToRoot` method can be updated to accept a callback function that we'll
pass the node into. Be sure to also pass the callback into the recursive
...
Finally, I added a simple wrapper method called `Tree.getRootNode`. Behind the
scenes, it will use the `Tree.walkToRoot` method, but this helps more clearly
indicate the intent when we call these methods in our code.
...
<Alert type="note">
  The full up to date Tree namespace script include can be found
