---
title: "ServiceNow Tree Traversal: Traversing to the Root Node"
slug: servicenow-tree-traversal-to-root-node
description:
  Creating a Tree traversal script include namespace in ServiceNow and functions
  to traverse to the root node of a tree
author: "Luke Covalt"
date: 2025-09-20
updated: 2025-09-21
draft: false
...
<Alert type="info">
	...
	Previous: [Introduction](/posts/servicenow-tree-traversal-introduction) |
	Next: [Pre-Order Depth-First Search](/posts/servicenow-tree-traversal-pre-order-depth-first-search)

    View the complete code on GitHub: [Tree.js](https://github.com/ltcovalt/covalt-dev/blob/main/src/content/scripts/servicenow/script-includes/Tree.js)

</Alert>

I'm going to start off with a simple function to get the root node from a child
...
(() => {
  /**
   * traverses from a lead to root node
   ...
   */
  Tree.traverseToRoot = ({ node, parentField = "parent" }) => {
    ...
    let root = Tree.traverseToRoot({ node: parentNode, parentField });
    return root;
  };
})();
```
...
would cause endless recursion. Since we're only traversing a single branch, we
...
(() => {
   ...
   * @param {string} [params.parentField=parent] - name of the parent reference field
   */
  Tree.traverseToRoot = ({ node, parentField = "parent", stackPath = [] }) => {
    ...
    // get the parent node, then call the function again using the parent node
    let parentNode = node[parentField].getRefRecord();
    return Tree.traverseToRoot({ node: parentNode, parentField, stackPath });
  };
})();
```
...
let root = Tree.traverseToRoot({ node });
...
want to perform some action on each node visited. To accomplish this, the
`Tree.traverseToRoot` method can be updated to accept a callback function that
we'll pass the node into. Be sure to also pass the callback into the recursive
...
We can write a simple test background script that prints the display value of
each visited node.
...
Finally, I added a simple wrapper method called `Tree.getRoot`. Behind the
scenes, it will use the `Tree.traverseToRoot` method, but this helps more
clearly indicate the intent when we call these methods in our code.
 ...
 */
Tree.getRoot = (params) => Tree.traverseToRoot(params);
```

<Alert type="note">
  The full Tree namespace script include can be found on github at
  [Tree.js](https://github.com/ltcovalt/covalt-dev/blob/main/src/content/scripts/servicenow/script-includes/Tree.js)
</Alert>
